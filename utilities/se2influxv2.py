...
class Se2Influx:
    ...
    def writer(self) -> None:
        ...
        with influxdb_client.InfluxDBClient(**self.influx_params.__dict__) as db_client:

            # Create the data bucket if it doesn't exist
            try:
                buckets_api = db_client.buckets_api()
                if self.influx_params.bucket not in list(map(lambda bucket: bucket.name,buckets_api.find_buckets().buckets)):
                    logging.info(
                        "Bucket %s missing; attempting to create", self.influx_params.bucket
                    )
                    buckets_api.create_bucket(bucket_name=self.influx_params.bucket, org=self.influx_params.org)
            except Exception as e:
                logging.error("Unable to connect to influxdb host or create bucket: %s", e)
                return
            ...
            while True:
                ...
                try:
                    ...
                    for point in data_out:
                        write_api.write(org=self.influx_params.org, bucket=self.influx_params.bucket,
                                record=point, 
                                record_time_stamp='time',
                                record_measurement_key='hw_type',
                                record_tag_keys=['serial'])
...
def parse_args() -> argparse.Namespace:
    ...
    db_args.add_argument(
            "--url", help="URL to connect to InfluxDB", default="http://localhost:8086"
    )
    db_args.add_argument("--token", help="api token")
    db_args.add_argument("--org", help="org to use")
    db_args.add_argument("--bucket", help="bucket to use", default='sedata')
    db_args.add_argument(
        "--timeout",
